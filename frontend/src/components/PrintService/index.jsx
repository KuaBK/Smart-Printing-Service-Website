...
export const PrintService = () => {
  ...
  const {fileChoice, setFileChoice} = useContext(GlobalContext);
  ...
  const handleFileUpload = async (event) => {
    Array.from(event.target.files).map(async (f) => {
    const file1 = f;
    ...
    try {
      ...
      const crsfToken = localStorage.getItem('CSRF_TOKEN');
    ...
    //     "numOfPage": 14,
    //     "shared": false,
    //     "url": "https://res.cloudinary.com/dawqytffc/image/upload/v1731944451/pf1m0awybd3jwd2csk4l.pdf",
    //     "uploadTime": "2024-11-18T15:40:51.163+00:00",
    //     "category": null,
    //     "fileConfig": null
    //  }
            ...
            <div className={classes.fileList}>
              {files.map((file, index) => (
                ...
                >
                  ...
                  <span className={classes.fileName}>{file.documentName}</span>
                  <span className={classes.fileSize}>{`${(file.size / 1024).toFixed(1)}KB`}</span>
